...
> **Implementation note.** The shipped tree follows this layout. Two details
> differ and are documented in [implementation-notes.md](implementation-notes.md):
> `models/` groups related entities per file (`identity.py`, `policy.py`,
> `session.py`, `registry.py`, `credential.py`, `audit_event.py`) rather than one
> file per table, and `tests/` sits at the repository root next to `agentid/`
...
Both are demonstrated by `python scripts/demo.py` and pinned by
`tests/e2e/test_killer_demo.py`.

---

# 23. Development Roadmap

See [roadmap.md](roadmap.md) for the current status of each phase.
...
[x] Policy engine
[x] Tool permissions
[x] Deny/allow decisions
[x] User → SA authorization
```
